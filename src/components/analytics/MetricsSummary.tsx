...
import { TrendingUp, TrendingDown, Minus, Users, Mail, MousePointer, Eye, ShoppingCart, Euro } from "lucide-react";
  ...
  const summaryMetrics = [
    {
      ...
      value: `${metrics.revenue?.toLocaleString("fr-FR") || 0}€`,
      icon: Euro,
      color: "text-success",
      change: calculateChange(metrics.revenue || 0, previousMetrics?.revenue),
    },
    {
      label: "Ventes",
      value: metrics.sales_count?.toString() || "0",
      icon: ShoppingCart,
      color: "text-primary",
      change: calculateChange(metrics.sales_count || 0, previousMetrics?.sales_count),
    },
    {
      ...
      change: calculateChange(metrics.conversion_rate || 0, previousMetrics?.conversion_rate),
    },
    {
      label: "Nouveaux inscrits",
      value: metrics.new_subscribers?.toString() || "0",
      icon: Users,
      color: "text-accent-foreground",
      change: calculateChange(metrics.new_subscribers || 0, previousMetrics?.new_subscribers),
    },
  ];
